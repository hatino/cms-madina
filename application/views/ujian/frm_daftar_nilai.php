...
<body>
    ...
    <form action="post" id="simpan_form">
        <div class="container mt-5">  
            <h3 class="text-header">Daftar Nilai</h3>
            ...
            <button type="button" id="btn_refresh" class="btn btn-sm btn-info text-white btn-shadow"><i class="bi bi-arrow-clockwise"></i> Refresh</button>
...
<script type="text/javascript">
    ...
    function fetch_tbl_daftar_nilai() {
        ...
        fetch('<?php echo site_url('ujian/daftar_nilai/get_tbl_daftar_nilai');?>?thnajaran='+thnajaran
                                                                        +'&jenjang='+jenjang
                                                                        +'&kelas='+kelas
                                                                        +'&subkelas='+subkelas
                                                                        +'&semester='+semester                                                                        
        ...
        {           
            ...
            for(var count = 0; count < data.length; count++) {     
                ...
                if(data[count].nilai==null){
                    ...
                    html += '   <td><button type="button" id="btn_detail" class="btn btn-sm btn-success btn-shadow" data-id="'+data[count].bank_soal_id+'" data-nis="'+data[count].nis+'" disabled>Detail</button></td>';//9      
                    html += '   <td><button type="button" id="btn_koreksi" class="btn btn-sm btn-primary btn-shadow" data-id="'+data[count].bank_soal_id+'" data-nis="'+data[count].nis+'" disabled>Koreksi</button></td>';//9      
                }else{
                    html += '   <td style="vertical-align: middle; min-width:100pt; text-align:center;">'+Math.trunc(data[count].nilai)+'</td>';//5                  
                    html += '   <td><button type="button" id="btn_detail" class="btn btn-sm btn-success btn-shadow" data-id="'+data[count].bank_soal_id+'" data-nis="'+data[count].nis+'">Detail</button></td>';//9      
                    html += '   <td><button type="button" id="btn_koreksi" class="btn btn-sm btn-primary btn-shadow" data-id="'+data[count].bank_soal_id+'" data-nis="'+data[count].nis+'">Koreksi</button></td>';//9  
                }          
    ...
    function validasi_refresh() {
        let valid=true;	
        var x = document.forms['simpan_form'];
        var jenjang = x['list_jenjang'].value;
        var kelas = x['list_kelas'].value;
        var subkelas = x['list_subkelas'].value;
        var semester = x['list_semester'].value;
        var jenis_penilaian = x['list_jenis_penilaian'].value;
        var matapel = x['list_mapel'].value;
        ...
        if (subkelas==''){
            $('#list_subkelas').css('border-color', '#cc0000')
            valid=false;	
        }
        if (semester==''){
    ...
    $(document).on('click', '#btn_detail', function () {      
        ...
        window.location.href="<?php echo site_url('ujian/daftar_nilai/show_daftar_nilai_detail') ;?>?bank_soal_id="+bank_soal_id+"&nis="+nis+"";
    })

     $(document).on('click', '#btn_koreksi', function () {      
        var bank_soal_id = $(this).attr('data-id')
        var nis = $(this).attr('data-nis')
        window.location.href="<?php echo site_url('ujian/daftar_nilai/show_koreksi_jawab_uraian') ;?>?bank_soal_id="+bank_soal_id+"&nis="+nis+"";
    })

    $(document).on('click', '#btn_refresh', function () {   
        if (validasi_refresh()==false){
