...
<script type="text/javascript">
    ...
    function fetch_daftar_soal_pg() {       
        fetch('<?php echo site_url('ujian/jawab_soal/get_data_soal_pg') ;?>?bank_soal_id='+bank_soal_id+'&username='+username+'')
        .then(response=> response.json())
        .then(resultData => {            
            var data  = resultData.data  
            if(data.length> 0){                
                ...
                for(var count = 0; count < data.length; count++) {     
                    ...
                    // html += '<tr id="'+ no +'" class="borderless-bottom" style="border-bottom: 20px solid transparent;">';                                          
                    // html += '   <td class="py-2"></td>';    
                    // if(data[count]['jawaban']=='e'){
                    //     html += '   <td class="py-0"><input class="chk_jawaban form-check-input" type="checkbox" name="chk_jawaban_'+data[count]['id']+'" data-id="'+data[count]['id']+'" id="'+data[count]['id']+'_chk_e" value="e" checked></td>'; 
                    // }else{
                    //     html += '   <td class="py-0"><input class="chk_jawaban form-check-input" type="checkbox" name="chk_jawaban_'+data[count]['id']+'" data-id="'+data[count]['id']+'" id="'+data[count]['id']+'_chk_e" value="e"></td>'; 
                    // }   
                    
                    // html += '   <td class="py-0"><label class="lbl_chk" for="'+data[count]['id']+'_chk_e">e</label></td>';  
                    // html += '   <td class="py-0"><label class="lbl_chk" for="'+data[count]['id']+'_chk_e"><div class="ck-content">'+data[count].jawaban_e+'</div></label></td>';
                    // html += '</tr>';       
                }      
