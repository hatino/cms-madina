...
<script type="text/javascript">
    ...
    async function fetch_data_adm() {
        ...
        .then( async (resultData) => {
            ...
            if(data.length> 0){
                ...
                thnajaran = await data[0]['thnajaran_cls']
                jenis_penilaian = await data[0]['jenis_penilaian']
                status_ekskul = await data[0]['status_ekskul']
    ...
    function fetch_jumlah_jawab_soal() {
        ...
        params.append('bank_soal_id', bank_soal_id)
        params.append('username', username)
        params.append('kelas', kelas)
        params.append('mapel_cls', mapel_cls)
        params.append('thnajaran', thnajaran)
        params.append('jenis_penilaian', jenis_penilaian)
        params.append('semester', semester)
                                                                    
        fetch('<?php echo site_url('ujian/jawab_soal/get_jumlah_jawab_soal_json') ;?>?'+params.toString()+'')
        .then(response => response.json())
        .then(resultData => {            
    ...
    function fetch_daftar_soal_pg() {       
        fetch('<?php echo site_url('ujian/jawab_soal/get_data_soal_pg') ;?>?bank_soal_id='+bank_soal_id
                                                                        +'&username='+username
                                                                        +'&kelas='+kelas
                                                                        +'&mapel_cls='+mapel_cls
                                                                        +'&thnajaran='+thnajaran
    ...
    function fetch_daftar_soal_essai() {
        fetch('<?php echo site_url('ujian/jawab_soal/get_data_soal_essai') ;?>?bank_soal_id='+bank_soal_id
                                                                            +'&username='+username
                                                                            +'&kelas='+kelas
                                                                            +'&mapel_cls='+mapel_cls
                                                                            +'&thnajaran='+thnajaran
        ...
        .then(resultData => {
            ...
            if(data.length> 0){                
                ...
                for(var count = 0; count < data.length; count++) {    
                    ...
                    html += '   <td></td>';    
                    if(data[count]['jawaban']==''||data[count]['jawaban']==null){
                        html += '   <td colspan="3"><textarea class="essai_jawaban form-control" type="text" name="essai_jawaban_'+data[count]['id']+'" id="jawaban_'+data[count]['id']+'" rows="3"></textarea></td>';
                    }else{
                    ...
                    html += '   <td></td>';                    
                    html += '   <td><button type="button" class="btn btn-sm btn-primary btn-shadow btn_simpan" id="'+data[count]['id']+'">Simpan</button></td>';                                                                                    
                    html += '</tr>';
                }
                html += '</tbody>';
                html += '</table>';    
    ...
    $(document).on('click', '.btn_simpan', async function () {   
        ...
        await simpan_jawaban_essai_kejson(soal_id, jawaban)
    })
    ...
    function simpan_jawaban_essai_kejson(soal_id, jawaban) {       
        const payload = {
        ...
        fetch("<?php echo site_url('ujian/jawab_soal/simpan_jawaban_essai_kejson'); ?>", {
            method: "POST",
            headers: {
